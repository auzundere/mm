...
const Uscalendarutils = {
	...
	calculateTimezoneAdjustment: function (event, date) {
		...
		if (event.start.tz) {
			...
			if (event.start.tz.includes(" ")) {
				...
				let tz = Uscalendarutils.getIanaTZFromMS(event.start.tz);
	...
	filterEvents: function (data, config) {
		...
		const eventDate = function (event, time) {
			return Uscalendarutils.isFullDayEvent(event) ? moment(event[time], "YYYYMMDD") : moment(new Date(event[time]));
		};
		...
		Object.entries(data).forEach(([key, event]) => {
			...
			if (event.type === "VEVENT") {
				...
				const title = Uscalendarutils.getTitleFromEvent(event);
				...
				for (let f in config.excludedEvents) {
					...
					if (Uscalendarutils.titleFilterApplies(testTitle, filter, useRegex, regexFlags)) {
						if (until) {
							dateFilter = until;
						} else {
							excluded = true;
				...
				if (typeof event.rrule !== "undefined" && event.rrule !== null && !isFacebookBirthday) {
					...
					let pastLocal = 0;
					let futureLocal = 0;
					if (Uscalendarutils.isFullDayEvent(event)) {
						Log.debug("fullday");
					...
					for (let d in dates) {
						...
						let adjustDays = Uscalendarutils.calculateTimezoneAdjustment(event, date);
						...
						const recurrenceTitle = Uscalendarutils.getTitleFromEvent(curEvent);
						...
						if (Uscalendarutils.timeFilterApplies(now, endDate, dateFilter)) {
							showRecurrence = false;
						}
						...
						if (showRecurrence === true) {
							...
							newEvents.push({
								...
								fullDayEvent: Uscalendarutils.isFullDayEvent(event),
								recurringEvent: true,
								class: event.class,
					...
					// Single event.
					const fullDayEvent = isFacebookBirthday ? true : Uscalendarutils.isFullDayEvent(event);
					...
					let adjustDays = Uscalendarutils.calculateTimezoneAdjustment(event, startDate.toDate());
...
if (typeof module !== "undefined") {
	module.exports = Uscalendarutils;
}
