...
const Homeworkutils = {
	...
	calculateTimezoneAdjustment: function (event, date) {
		...
		if (event.start.tz) {
			...
			if (event.start.tz.includes(" ")) {
				...
				let tz = Homeworkutils.getIanaTZFromMS(event.start.tz);
	...
	filterEvents: function (data, config) {
		...
		const eventDate = function (event, time) {
			return Homeworkutils.isFullDayEvent(event) ? moment(event[time], "YYYYMMDD") : moment(new Date(event[time]));
		};
		...
		Object.entries(data).forEach(([key, event]) => {
			...
			if (event.type === "VEVENT") {
				...
				const title = Homeworkutils.getTitleFromEvent(event);
				...
				for (let f in config.excludedEvents) {
					...
					if (Homeworkutils.titleFilterApplies(testTitle, filter, useRegex, regexFlags)) {
						if (until) {
							dateFilter = until;
						} else {
							excluded = true;
				...
				if (typeof event.rrule !== "undefined" && event.rrule !== null && !isFacebookBirthday) {
					...
					let pastLocal = 0;
					let futureLocal = 0;
					if (Homeworkutils.isFullDayEvent(event)) {
						Log.debug("fullday");
					...
					for (let d in dates) {
						...
						let adjustDays = Homeworkutils.calculateTimezoneAdjustment(event, date);
						...
						const recurrenceTitle = Homeworkutils.getTitleFromEvent(curEvent);
						...
						if (Homeworkutils.timeFilterApplies(now, endDate, dateFilter)) {
							showRecurrence = false;
						}
						...
						if (showRecurrence === true) {
							...
							newEvents.push({
								...
								fullDayEvent: Homeworkutils.isFullDayEvent(event),
								recurringEvent: true,
								class: event.class,
					...
					// Single event.
					const fullDayEvent = isFacebookBirthday ? true : Homeworkutils.isFullDayEvent(event);
					...
					let adjustDays = Homeworkutils.calculateTimezoneAdjustment(event, startDate.toDate());
