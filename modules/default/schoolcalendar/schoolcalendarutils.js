...
const Schoolcalendarutils = {
	...
	calculateTimezoneAdjustment: function (event, date) {
		...
		if (event.start.tz) {
			...
			if (event.start.tz.includes(" ")) {
				...
				let tz = Schoolcalendarutils.getIanaTZFromMS(event.start.tz);
	...
	filterEvents: function (data, config) {
		...
		const eventDate = function (event, time) {
			return Schoolcalendarutils.isFullDayEvent(event) ? moment(event[time], "YYYYMMDD") : moment(new Date(event[time]));
		};
		...
		Object.entries(data).forEach(([key, event]) => {
			...
			if (event.type === "VEVENT") {
				...
				const title = Schoolcalendarutils.getTitleFromEvent(event);
				...
				for (let f in config.excludedEvents) {
					...
					if (Schoolcalendarutils.titleFilterApplies(testTitle, filter, useRegex, regexFlags)) {
						if (until) {
							dateFilter = until;
						} else {
							excluded = true;
				...
				if (typeof event.rrule !== "undefined" && event.rrule !== null && !isFacebookBirthday) {
					...
					let pastLocal = 0;
					let futureLocal = 0;
					if (Schoolcalendarutils.isFullDayEvent(event)) {
						Log.debug("fullday");
					...
					for (let d in dates) {
						...
						let adjustDays = Schoolcalendarutils.calculateTimezoneAdjustment(event, date);
						...
						const recurrenceTitle = Schoolcalendarutils.getTitleFromEvent(curEvent);
						...
						if (Schoolcalendarutils.timeFilterApplies(now, endDate, dateFilter)) {
							showRecurrence = false;
						}
						...
						if (showRecurrence === true) {
							...
							newEvents.push({
								...
								fullDayEvent: Schoolcalendarutils.isFullDayEvent(event),
								recurringEvent: true,
								class: event.class,
					...
					// Single event.
					const fullDayEvent = isFacebookBirthday ? true : Schoolcalendarutils.isFullDayEvent(event);
					...
					let adjustDays = Schoolcalendarutils.calculateTimezoneAdjustment(event, startDate.toDate());
...
if (typeof module !== "undefined") {
	module.exports = Schoolcalendarutils;
}
